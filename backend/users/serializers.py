...
class TeacherCreateSerializer(serializers.Serializer):
    ...
    full_name = serializers.CharField(max_length=225, required=False, allow_blank=True)
    phone = serializers.CharField(max_length=20, required=False, allow_blank=True)
    qualification = serializers.CharField(max_length=225, required=False, allow_blank=True)
    joining_date = serializers.DateTimeField(required=False, allow_null=True)
    ...
    def create(self, validated_data):
        ...
        if not admin_user or not admin_user.tenant:
            raise serializers.ValidationError('Admin user must belong to a tenant.')
        ...
        with transaction.atomic():
            ...
            message = (
                ...
                f"You have been added as a teacher at {admin_user.tenant.institute_name}.\n\n"
                f"Login details:\n"
                f"Username: {user.email}\n"
                f"Password: {password}\n\n"
                "Please login and update your profile immediately. For security, change your password.\n\n"
                "Regards,\nEduQuest Team"
            )
            ...
            try:
                ...
                raise serializers.ValidationError(
                    {"email": f"Failed to send credentials email: {str(e)}"}
                )
...
class StudentCreateSerializer(serializers.Serializer):
    ...
    roll_number = serializers.IntegerField(min_value=0)
    ...
    def create(self, validated_data):
        ...
        if not admin_user.tenant:
            raise serializers.ValidationError('Admin must belong to a tenant.')
        ...
        with transaction.atomic():
            ...
            try:
                ...
                raise serializers.ValidationError('Failed to send email. Student not created.')
...
class StudentListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Student
        fields = ["id", "full_name", "email", "admission_number", "class_id", "roll_number"]
